...
while True:
	...
	if detect == ord('5'): #take a screenshot if '5' is pressed
		...
		counter[4]+=1
	if detect == ord('6'): #take a screenshot if '6' is pressed
		cv2.imwrite('images/'+str(index)+'.step/background'+str(index)+str(int(counter[5]))+'.bmp',image) #save screenshot as last.bmp
		counter[5]+=1
	#show image
